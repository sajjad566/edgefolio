...
const NavLink = ({ name, Icon, text, slug, isOpen, toggleMenu, ...rest }) => (
  <ActiveLink
    href={slug}
...
const Menu = () => {
  ...
  return (
    <>
      <div
        className={classNames(
          'fixed top-0 left-0 z-10 block h-screen w-full animate-fade-in bg-black/90 md:hidden',
          isOpen ? 'h-screen' : 'h-0'
        )}
        onClick={closeMenu}
      />
      ...
      >
        {menu && (
          <div className="grid w-full grid-cols-4 bg-gradient-to-b from-transparent to-omega-800 md:block">
            {menu &&
              menu.map((item) => <NavLink {...item} key={`${item.slug}`} onClick={closeMenu} />)}
            <div
              className={classNames(
